...
impl<Optic, S> PatternMatch<Optic, S> {
    pub fn new(camera: Camera<Optic>, searcher: S, max_iters: usize) -> Self {
        Self {
            camera,
            searcher,
            max_iters,
...
impl<Optic, S: Searcher> Estimator<SensorFrame> for PatternMatch<Optic, S> {
    type Output = Orientation<Ecef>;
    ...
    fn estimate<I: RayIterator<SensorFrame>>(self, rays: I) -> Self::Output {
        ...
        //         let cam = Optic::new(self.lens.clone(), ort.clone());
        //
        //         // Find the zenith coordinate in CameraFrd.
        //         let zenith_coord = cam
        //             .trace_from_sky(
        //                 Bearing::<Ecef>::builder()
        //                     .azimuth(Angle::ZERO)
...
pub trait Searcher {
    type Iter: Iterator<Item = Orientation<Ecef>>;

    fn orientations(self) -> Self::Iter;
}

pub struct VecSearch {
    orts: Vec<Orientation<Ecef>>,
}

impl VecSearch {
    pub fn new(orts: Vec<Orientation<Ecef>>) -> Self {
        Self { orts }
    }
}

impl Searcher for VecSearch {
    type Iter = std::vec::IntoIter<Orientation<Ecef>>;
